...
@Service(SeerQualifier.AGV_CLIENT_NAVI_WORKER)
public class SeerAgvClientNaviWorker implements IConnectWorker {
	...
	public synchronized void init() {
		...
		for (IotClientBean bean : iotClientService.getList()) {
			if (!bean.getDevtype().equals(IotDevType.AGV_SEER_NAVI)) {
				continue;
			}
			try {
				IConnect client;
