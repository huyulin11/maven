...
@Service(SeerQualifier.AGV_CLIENT_CTRL_WORKER)
public class SeerAgvClientCtrlWorker implements IConnectWorker {
	...
	public synchronized void init() {
		...
		for (IotClientBean bean : iotClientService.getList()) {
			if (!bean.getDevtype().equals(IotDevType.AGV_SEER_CTRL)) {
				continue;
			}
			try {
				IConnect client;
