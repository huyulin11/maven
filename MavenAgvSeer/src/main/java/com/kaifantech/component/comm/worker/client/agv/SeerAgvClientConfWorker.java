...
@Service(SeerQualifier.AGV_CLIENT_CONF_WORKER)
public class SeerAgvClientConfWorker implements IConnectWorker {
	...
	public synchronized void init() {
		...
		for (IotClientBean bean : iotClientService.getList()) {
			if (!bean.getDevtype().equals(IotDevType.AGV_SEER_CONF)) {
				continue;
			}
			try {
				IConnect client;
