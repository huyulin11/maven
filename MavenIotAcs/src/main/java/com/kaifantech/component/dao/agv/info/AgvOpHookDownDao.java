...
@Service
public class AgvOpHookDownDao extends AgvInfoDao {
	@Autowired
	@Qualifier(UdfQualifier.DEFAULT_CTRL_MODULE)
	private ICtrlModule ctrlModule;

	public void reach(Integer agvId) {
		update(agvId, " sitestatus='" + AgvSiteStatus.HOOK_DOWN + "'",
				AND + " sitestatus='" + AgvSiteStatus.MOVING + "'", AND + "(" + " taskstatus='" + AgvTaskType.GOTO_INIT
						+ "'" + " OR " + " taskstatus='" + AgvTaskType.DELIVER + "'" + ")");
	}

	public void over(Integer agvId) {
		update(agvId, " sitestatus='" + AgvSiteStatus.MOVING + "'",
				AND + " sitestatus='" + AgvSiteStatus.HOOK_DOWN + "'", AND + "(" + " taskstatus='"
						+ AgvTaskType.GOTO_INIT + "'" + " OR " + " taskstatus='" + AgvTaskType.DELIVER + "'" + ")");
