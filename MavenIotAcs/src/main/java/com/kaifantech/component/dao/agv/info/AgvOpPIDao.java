...
@Service
public class AgvOpPIDao extends AgvInfoDao {
	@Autowired
	@Qualifier(UdfQualifier.DEFAULT_CTRL_MODULE)
	private ICtrlModule ctrlModule;

	public void reach(Integer agvId) {
		update(agvId, " sitestatus=" + str(AgvSiteStatus.PI), AND + " sitestatus=" + str(AgvSiteStatus.MOVING));
	}

	public void over(Integer agvId) {
		update(agvId, " sitestatus=" + str(AgvSiteStatus.MOVING), AND + " sitestatus=" + str(AgvSiteStatus.PI));
